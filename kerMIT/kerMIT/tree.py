__author__ = 'Lorenzo, Leonardo Ranaldi'
...
class Tree:
    ...
    # returns a dictionary where each node id is associated with the corresponding root node
    ...
    # returns a dictionary where the key is a node and the value is the parent
    ...
    # function that returns the list of nodes needed to get from itself to the root of the tree the path
    def getPathFromALeaveToRoot(self, dict_parent, dict_index):
        """

        :param dict_parent: child-parent dictionary
        :param dict_index: index child-parent dictionary
        :return: list of nodes needed to get from itself to the root of the tree the path
        """
        start = self._id
        path = []
        while start != None:
    ...
    def matrixToString(self,matrix_paths):
        ...
        :param matrix_paths: matrix where each row is a list of leaf-root paths (e.g. [the, DT, NP])
        :return: list in which in each cell there is the path in tree form (ex: (NP (DT the)) )
        """
        matrix_tree = []
    ...
    # given a tree object, returns all paths from the tree leaves to the root
